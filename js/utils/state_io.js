...
(() => {
  ...
  SubPub.subscribe({
    events: ["state::patch::chapter_filter::received"],
      listener: ({params}) => {
        State.filter = params;
        SubPub.publish({
    ...
    events.forEach((event) => {
      SubPub.subscribe({
        ...
        listener: ({ response, params }) => {
          ...
          response.elements &&
            fix_ints_and_booleans_elements(response.elements);
...
function cascade_delete(entity, id) {
  ...
  if (index !== -1) {
    const next_entity =
      index >= entities.length - 1 ? false : entities[index + 1];
    delete_element(entity + "s", id);

    if (next_entity) {
      State[next_entity + "s"]
        .filter((x) => x[entity + "_id"] === id)
        .forEach((x) => cascade_delete(next_entity, x[next_entity + "_id"]));
    }
  }
  ...
  if (entity === "unit") {
    State.dependencies &&
      delete_elements(
        State.dependencies.filter((x) => x.unit_id === id),
        State.dependencies,
        "unit_id"
      );
    State.users_units &&
      delete_elements(
        State.users_units.filter((x) => x.unit_id === id),
        State.users_units,
        "unit_id"
      );
  }
  ...
  if (entity === "user") {
    State.users_units &&
      delete_elements(
        State.users_units.filter((x) => x.user_id === id),
        State.users_units,
        "user_id"
      );
  }
  ...
  if (entity === "section") {
    State.dependencies &&
      delete_elements(
        State.dependencies.filter((x) => x.section_id === id),
        State.dependencies,
        "section_id"
      );
  }
...
function is_unit_empty(element) {
  return (
    (element.kind !== "quiz" && !element.story) ||
    (element.kind === "quiz" && element.name !== "Done")
  );
}

function get_class_expanded_chapter(element) {
  const expanded_chapters = JSON.parse(
    localStorage.getItem("expanded_chapters")
  );
  return !!expanded_chapters?.find(
    (c) =>
      c.chapter_id === element.chapter_id && c.course_id === element.course_id
  );
}
function set_class_expanded_chapter(element, expanded) {
  const expanded_chapters =
    JSON.parse(localStorage.getItem("expanded_chapters")) || [];
  if (expanded) {
    expanded_chapters.push(element);
  } else {
    const index = expanded_chapters.find(
      (c) =>
        c.chapter_id === element.chapter_id && c.course_id === element.course_id
    );
    if (index !== -1) {
...
function get_chapter_units(chapter_id) {
  let units = [];
  get_chapter_sections(chapter_id).forEach(
    (s) => (units = [...units, ...get_section_units(s.section_id)])
  );
  return units;
}
...
function get_quiz_question_status(question) {
  ...
  if (answers.length) {
    status = answers.some((answer) => is_answer_correct({ answer }))
      ? "answered_correct"
      : "answered_incorrect";
  }
  return status;
...
function get_question_answers({ question }) {
  return State.quiz_answers.filter(
    (a) => a.quiz_question_id === question.quiz_question_id
  );
}
function is_answer_correct({ answer }) {
  const option = State.quiz_options.find(
    (op) => op.quiz_option_id === answer.quiz_option_id
  );
